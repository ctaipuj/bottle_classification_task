...
vector<vector<double> > space::find_depth(vector<cv::Point> Points_scaled, vector<cv::Point> Points_real, cv::Mat depth, vector<vector<cv::Point> > &contornos){
	...
	for (int i =0;i<Points_real.size();i++){
		if(!isnan(depth.at<uint16_t>(Points_scaled[i]))){//|| depth.at<float>(Points_scaled[i])!=0){
			uvz[i][0]=Points_real[i].x;
			uvz[i][1]=Points_real[i].y;
			uvz[i][2]=depth.at<uint16_t>(Points_scaled[i]);
		}
	}
	return uvz;
}
...
vector<vector<double> > space::xyz_coord(vector<vector<double> > Points_xyz, double fx, double fy, double cx, double cy){
	...
	for (int i=0;i<Points_xyz.size();i++){
		kinect_coord[i][0]= (Points_xyz[i][2]*((Points_xyz[i][0]-cx)/fx))/10;//dividir en 10 para obtener cm 
		kinect_coord[i][1]= (Points_xyz[i][2]*((Points_xyz[i][1]-cy)/fy))/10;
		kinect_coord[i][2]= Points_xyz[i][2]/10;
	}
	return kinect_coord;
}
